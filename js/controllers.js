...
controller('rawCtrl', function($scope, $http, dataService) {
        ...
        chartsUrl.forEach(function(d, i) {
            ...
            function loaded() {
                if (i == chartsUrl.length - 1) {
                    $scope.charts = d3.values(raw.charts).map(function(chart) {
                        return chart();
                    });
                    $scope.charts.forEach(function(c, ind) {
                        c.id = ind;
                        c.text = c.title;
                    })
    ...
    $scope.$watch("text", function() {
        ...
        try {
            ...
            $scope.header = d3.keys($scope.data[0]).map(function(d) {
                return {
                    key: d,
                    type: raw.maxOnValue(raw.count(sniff[d]))
                }
            })
        } catch (e) {
            $scope.data = [];
    ...
    $scope.reset = function() {
        ...
        $scope.charts.forEach(function(chart) {
            ...
            d3.values(chart.model.structure).forEach(function(d) {
                d.value = [];
            });
            // reset model map
            d3.values(chart.model.map).forEach(function(d) {
                d.value = [];
            });
        })
    }
